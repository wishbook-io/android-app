...
public class SellerManifestAPIHandler {
    ...
    HashMap<String, String> manifest_image_successcount;

    public static Set<SellerManifestAPIHandler.SellerManifestAPIListener> apiHandlerListener;


    public SellerManifestAPIHandler(Activity context) {
        this.context = context;
    }

    public void callManifestImageUpload(ResponseManifestList sellerInvoiceResponse) {
        ...
        if (toupload_imagesArrayList != null & toupload_imagesArrayList.size() > 0) {
            String manifest_id = sellerInvoiceResponse.getId();
            showUploadDialog();
            for (int position = 0; position < toupload_imagesArrayList.size(); position++) {
                HashMap params = new HashMap();
                params.put("manifest", manifest_id);
                ...
                HttpManager.getInstance(context).requestwithFile(HttpManager.METHOD.FILEUPLOAD, URLConstants.companyUrl(context, "manifest-images", ""), params, headers, "image", "image/jpg", outputrenamed, false, new HttpManager.customCallBack() {
                    ...
                    public void onServerResponse(String response, boolean dataUpdated) {
                        try {
                            if (manifest_image_successcount == null) {
                                manifest_image_successcount = new HashMap<>();
                            }
                            manifest_image_successcount.put(String.valueOf(finalPosition), "true");
                            Log.e("TAG", "onServerResponse: Put Position" + finalPosition);
                            boolean isComplete = observalImageSuccessCount(manifest_image_successcount, toupload_imagesArrayList);
                            if (isComplete) {
                                hideUploadDialog();
                                callManifetsUpdateCopy(manifest_id);
                            } else {
    ...
    public void callManifetsUpdateCopy(String manifest_id) {
        HashMap<String, String> headers = StaticFunctions.getAuthHeader((Activity) context);
        String url = URLConstants.companyUrl(context, "manifests", "") + manifest_id + "/";
        HashMap<String, String> param = new HashMap<>();
        param.put("id", manifest_id);
        param.put("update_signed_copy", "true");
        HttpManager.getInstance(context).requestwithObject(HttpManager.METHOD.PATCHJSONOBJECTWITHPROGRESS, url, new Gson().fromJson(new Gson().toJson(param), JsonObject.class), headers, false, new HttpManager.customCallBack() {
            ...
            public void onServerResponse(String response, boolean dataUpdated) {
                if (SellerManifestAPIHandler.apiHandlerListener != null) {
                    for (SellerManifestAPIHandler.SellerManifestAPIListener item :
                            SellerManifestAPIHandler.apiHandlerListener) {
    ...
    public void setSellerInvoiceAPIListener(SellerManifestAPIHandler.SellerManifestAPIListener sellerInvoiceAPIListener) {
        if (SellerManifestAPIHandler.apiHandlerListener == null) {
            SellerManifestAPIHandler.apiHandlerListener = new HashSet<>();
        }
        SellerManifestAPIHandler.apiHandlerListener.add(sellerInvoiceAPIListener);

    }


    public interface SellerManifestAPIListener {
    ...
    private boolean observalImageSuccessCount(HashMap<String, String> hashmap, ArrayList<SellerInvoice_images> sellerInvoice_imagesArrayList) {
