...
/**
 * Created by vignesh on 6/8/13.
 */
class NetworkProcessorDrawable extends LayerDrawable {
    ...
    private NetworkProcessor ion;
    private BitmapFetcher bitmapFetcher;
    private NetworkProcessorDrawableCallback callback;
    private FutureCallback<NetworkProcessorDrawable> loadCallback;
    private NetworkProcessorGifDecoder gifDecoder;
    ...
    public FutureCallback<NetworkProcessorDrawable> getLoadCallback() {
        return loadCallback;
    }

    public void setLoadCallback(FutureCallback<NetworkProcessorDrawable> loadCallback) {
        this.loadCallback = loadCallback;
    }

    public NetworkProcessorDrawable ion(NetworkProcessor ion) {
        if (ion == null)
    ...
    static class NetworkProcessorDrawableCallback implements FutureCallback<BitmapInfo> {
        private WeakReference<NetworkProcessorDrawable> ionDrawableRef;
        private String bitmapKey;
        private NetworkProcessor ion;
        public NetworkProcessorDrawableCallback(NetworkProcessorDrawable drawable) {
            ionDrawableRef = new WeakReference<NetworkProcessorDrawable>(drawable);
//            imageViewRef = new ContextReference.ImageViewContextReference(imageView);
        }

        public void register(NetworkProcessor ion, String bitmapKey) {
            String previousKey = this.bitmapKey;
            NetworkProcessor previousNetworkProcessor = this.ion;
            ...
            unregister(previousNetworkProcessor, previousKey);
        }

        private void unregister(NetworkProcessor ion, String key) {
            if (key == null)
                return;
        ...
        public void onCompleted(Exception e, BitmapInfo result) {
            ...
            NetworkProcessorDrawable drawable = ionDrawableRef.get();
            if (drawable == null)
                return;
            drawable
            .setBitmap(result, result.servedFrom)
            .updateLayers();
            FutureCallback<NetworkProcessorDrawable> callback = drawable.loadCallback;
            if (callback != null)
                callback.onCompleted(e, drawable);
        }
    }

    class NetworkProcessorGifDecoder {
        GifDecoder gifDecoder;
        Exception exception;
        GifFrame currentFrame;
        long nextFrameRender;
        public NetworkProcessorGifDecoder(BitmapInfo info){
        ...
        Runnable loader = new Runnable() {
            ...
            public void run() {
                ...
                NetworkProcessor.mainHandler.post(postLoad);
            }
        };
        ...
        public synchronized void scheduleNextFrame() {
            ...
            if (gifDecoder.getStatus() == GifDecoder.STATUS_FINISH && repeatAnimation)
                gifDecoder.restart();
            isLoading = true;
            NetworkProcessor.getBitmapLoadExecutorService().execute(loader);
        }
    }

    public NetworkProcessorDrawable setFadeIn(boolean fadeIn) {
        this.fadeIn = fadeIn;
        return this;
    }

    public NetworkProcessorDrawable setBitmapFetcher(BitmapFetcher bitmapFetcher) {
    ...
    public NetworkProcessorDrawable setBitmapDrawableFactory(BitmapDrawableFactory factory) {
    ...
    public NetworkProcessorDrawable(Resources resources) {
        ...
        callback = new NetworkProcessorDrawableCallback(this);
    }

    public NetworkProcessorDrawable updateLayers() {
    ...
    public NetworkProcessorDrawable setBitmap(BitmapInfo info, ResponseServedFrom servedFrom) {
        if (this.info == info)
            return this;
        ...
        if (info.decoder != null) {
            ...
            gifDecoder = new NetworkProcessorGifDecoder(info);
        }

        return this;
    }

    public NetworkProcessorDrawable setRepeatAnimation(boolean repeatAnimation) {
        this.repeatAnimation = repeatAnimation;
        return this;
    }

    public NetworkProcessorDrawable setSize(int resizeWidth, int resizeHeight) {
    ...
    public NetworkProcessorDrawable setError(int resource, Drawable drawable) {
    ...
    public NetworkProcessorDrawable setPlaceholder(int resource, Drawable drawable) {
...
//                    System.out.println(String.format("falling back for %s,%s,%s to %s,%s,%s: %s,%s (%s to %s)", x, y, level, parentX, parentY, parentLevel, parentLeft, parentTop, subTextureDim, subTextureDim >> parentUp));
    ...
    static NetworkProcessorDrawable getOrCreateNetworkProcessorDrawable(ImageView imageView) {
        Drawable current = imageView.getDrawable();
        NetworkProcessorDrawable ret;
        if (current == null || !(current instanceof NetworkProcessorDrawable))
            ret = new NetworkProcessorDrawable(imageView.getResources());
        else
            ret = (NetworkProcessorDrawable)current;
