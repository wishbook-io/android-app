...
import static com.wishbook.catalog.Utils.networking.async.http.spdy.Settings.DEFAULT_INITIAL_WINDOW_SIZE;

/**
 * Created by vignesh on 7/16/14.
 */
public class AsyncSpdyConnection implements FrameReader.Handler {
    ...
    private SpdySocket newStream(int associatedStreamId, List<Header> requestHeaders, boolean out,
        ...
        nextStreamId += 2;
        socket = new SpdySocket(streamId);
        if (socket.isOpen()) {
...
//            return;
    ...
    @Override
    public void headers(boolean inFinished, int streamId, List<Header> headerBlock, HeadersMode headersMode) {
        if (pushedStream(streamId)) {
            throw new AssertionError("push");
...
//            return;
        ...
        socket.receiveHeaders(headerBlock);
        if (inFinished) {
            sockets.remove(streamId);
            Util.end(socket, null);
        }
...
//            return;
    ...
    boolean shutdown;
    @Override
    public void goAway(int lastGoodStreamId) {
        shutdown = true;
    ...
    @Override
    public void priority(int streamDependency, int weight, boolean exclusive) {
    }
