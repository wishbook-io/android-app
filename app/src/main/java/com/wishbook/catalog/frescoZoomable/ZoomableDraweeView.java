...
public class ZoomableDraweeView extends DraweeView<GenericDraweeHierarchy>
    ...
    private static final float HUGE_IMAGE_SCALE_FACTOR_THRESHOLD = 1.3f;
    ...
    private ProductDoubleTabListner doubleTapGestureListener1;

    private boolean mAllowTouchInterceptionWhileZoomed = true;
    private boolean isZoomResetAvailable = false;
    ...
    private final ZoomableController.Listener mZoomableListener =
            new ZoomableController.Listener() {
                ...
                public void onTransformEnd(Matrix transform) {
                    Log.e("TAG", "onTransformEnd: >>>>" );
                }

                @Override
                public void onResetZoom() {
                    if(doubleTapGestureListener!=null) {
                        doubleTapGestureListener.callOnResume();
                    }
                }
            };
    ...
    public void setTapListener(GestureDetector.SimpleOnGestureListener tapListener) {
        doubleTapGestureListener = (DoubleTapGestureListener) tapListener;
        mTapListenerWrapper.setListener(tapListener);
    }

    /**
     * Sets the tap listener Products.
     */
    public void setTapListener1(GestureDetector.SimpleOnGestureListener tapListener) {
        doubleTapGestureListener1 = (ProductDoubleTabListner) tapListener;
        mTapListenerWrapper.setListener(tapListener);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(event.getActionMasked() == MotionEvent.ACTION_CANCEL){
            if(mZoomableListener!=null){
                mZoomableListener.onResetZoom();
            }
        }
        Log.e("TAG", "onTouchEvent: Zoomable" );
    ...
    public void setIsZoomResetAvailable( boolean isZoomResetAvailable){
        this.isZoomResetAvailable = isZoomResetAvailable;
        if(mZoomableController!=null)
        {
            mZoomableController.setResetAvailable(isZoomResetAvailable);
        }
    }
