...
//        transaction.add(R.id.emojicons_frame_layout, emojiFragment).commit();
    ...
    protected void processSendMessage() {
        if (!TextUtils.isEmpty(messageEditText.getText().toString().trim()) || !TextUtils.isEmpty(filePath)) {
            ...
            if (disjointResult) {
                String orignal_one = messageEditText.getText().toString().trim();
                //String email_filtermessage = orignal_one.replaceAll("^([a-zA-Z0-9_\\-\\.]+)@((\\[[0-9]{1,3}\\.[0-9]{1,3}\\.[0-9]{1,3}\\.)|(([a-zA-Z0-9\\-]+\\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\\]?)$","*****");
               // String email_filtermessage = orignal_one.replaceAll("[a-zA-Z0-9._-]+@[a-z]+\\\\.+[a-z]+","email");
                //String website_filtermessage = email_filtermessage.replaceAll("^([a-zA-Z0-9_\\-\\.]+)@((\\[[0-9]{1,3}\\.[0-9]{1,3}\\.[0-9]{1,3}\\.)|(([a-zA-Z0-9\\-]+\\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\\]?)$","*****");
                //String filter_message = website_filtermessage.replaceAll("[\\.)(]*([0-9+-.]{1,2})[\\.)( ]*([0-9+-.]{1,4})[\\.)( ]*([0-9+-.]{1,4})[\\.)( ]*([0-9+-.]{1,4})[\\.)( ]*([0-9+-.]{1,4})[\\.)( ]*([0-9+-.]{1,4})[\\.)( ]*([0-9+-.]{1,4})","****");

                String email_filter_msg = orignal_one.replaceAll("[a-zA-Z0-9._-]+@[a-z]+.[a-z]+","***");
                String url_filter_msg = email_filter_msg.replaceAll("((http:\\/\\/|https:\\/\\/)?(www.)?(([a-zA-Z0-9-]){2,}\\.){1,4}([a-zA-Z]){2,6}(\\/([a-zA-Z-_\\/\\.0-9#:?=&;,]*)?)?)","***");
                String phone_filter_msg = url_filter_msg.replaceAll("[\\.)(]*([0-9+-.]{1,2})[\\.)( ]*([0-9+-.]{1,4})[\\.)( ]*([0-9+-.]{1,4})[\\.)( ]*([0-9+-.]{1,4})[\\.)( ]*([0-9+-.]{1,4})[\\.)( ]*([0-9+-.]{1,4})[\\.)( ]*([0-9+-.]{1,4})","****");
                sendMessage(phone_filter_msg);
...
//    }
    ...
    public void sendMessage(String message) {
        ...
        sendMessage.putExtra("action", "sendMessage");
        LocalBroadcastManager.getInstance(getActivity()).sendBroadcast(sendMessage);
    }
